 ...
 */
export function filterVendorsByCategory(vendors: Vendor[], category: string): Vendor[] {
  if (!category || category === 'all') return vendors;

  const normalizedCategory = category.toLowerCase().trim();
  return vendors.filter((vendor) =>
    vendor.categories.some((cat) => cat.toLowerCase().trim() === normalizedCategory)
  );
}
...
export function sortVendors(
  ...
  switch (sortKey) {
    ...
    case 'createdAt':
      sorted.sort((a, b) => {
        const aTime = typeof a.createdAt === 'bigint' ? Number(a.createdAt) : (a.createdAt as number);
        const bTime = typeof b.createdAt === 'bigint' ? Number(b.createdAt) : (b.createdAt as number);
        return bTime - aTime;
      });
      break;
