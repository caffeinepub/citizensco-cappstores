...
/**
 * Filter state shape used by the vendor directory page.
 */
...
/**
 * Reads vendor directory filter state from the current URL query string.
 * Falls back to sensible defaults when parameters are absent.
 *
 * @returns The current filter state derived from URL params.
 */
export function readFiltersFromUrl(): VendorDirectoryFilters {
    const params = new URLSearchParams(window.location.search);

    const search = params.get('search') ?? '';
    const category = params.get('category') ?? null;
    const rawSort = params.get('sort');
    const sort: 'name-asc' | 'newest' =
        rawSort === 'newest' ? 'newest' : 'name-asc';

    return { search, category, sort };
}

/**
 * Updates the browser URL query string with the given filter values without
 * triggering a page reload. Null / empty values are omitted from the URL.
 *
 * @param filters - Partial filter state to persist in the URL.
 */
