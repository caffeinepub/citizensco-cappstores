...
export function validateName(name: string): ValidationError | null {
  ...
  if (trimmed.length > NAME_MAX_LENGTH) {
    return { field: 'name', message: `Name must not exceed ${NAME_MAX_LENGTH} characters` };
  }
...
export function validateEmail(email: string): ValidationError | null {
  ...
  if (trimmed.length < EMAIL_MIN_LENGTH) {
    return { field: 'email', message: `Email must be at least ${EMAIL_MIN_LENGTH} characters` };
  }
  
  if (trimmed.length > EMAIL_MAX_LENGTH) {
    return { field: 'email', message: `Email must not exceed ${EMAIL_MAX_LENGTH} characters` };
  }
  
  // Basic email format validation
  const parts = trimmed.split('@');
  if (parts.length !== 2) {
    return { field: 'email', message: 'Email must contain exactly one @ symbol' };
  }
 ...
 */
export function validateProfile(profile: { name: string; email?: string }): ValidationError[] {
