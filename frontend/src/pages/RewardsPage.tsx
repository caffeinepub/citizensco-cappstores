...
import { Gift, Trophy, Users, GraduationCap, Coins, Star, Zap, Heart, Wrench, Brain, Sparkles, Loader2, CheckCircle2 } from 'lucide-react';
import { toast } from 'sonner';
import { RewardCampaign, RewardCampaignType } from '../backend';
...
function CampaignCard({ campaign, userPrincipal, pendingCampaigns, joinCampaign, completeCampaign, onJoin, onComplete }: CampaignCardProps) {
  const typeKey = campaign.campaignType as unknown as string;
  const Icon = campaignTypeIcons[typeKey] || Gift;
  const colorClass = campaignTypeColors[typeKey] || 'bg-gray-500/10 text-gray-600 border-gray-500/20';
  ...
  const isJoining = joinCampaign.isPending && joinCampaign.variables === campaign.id;
  const isCompleting = completeCampaign.isPending && completeCampaign.variables === campaign.id;

  return (
    <Card className={`hover:shadow-lg transition-all duration-300 border-2 ${colorClass}`}>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{campaign.participants.length} participant{campaign.participants.length !== 1 ? 's' : ''}</span>
        </div>
    ...
    <div className="container py-12">
      ...
      {!campaignsLoading && !campaignsError && (
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <Card className="text-center p-4">
            <div className="text-2xl font-bold text-chart-2">{activeCampaignsCount}</div>
            <div className="text-sm text-muted-foreground">Pending Rewards</div>
          </Card>
          <Card className="text-center p-4">
            <div className="text-2xl font-bold text-chart-3">{completedCampaignsCount}</div>
          ...
          {campaignsLoading ? (
            ...
            <Card className="text-center py-12 border-destructive/50">
              <CardContent>
                ...
                <p className="text-lg font-semibold mb-1 text-destructive">Error Loading Campaigns</p>
                <p className="text-muted-foreground">
