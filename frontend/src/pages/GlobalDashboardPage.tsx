...
import { useGetAllAnalytics, useGetProjectEntries, useGetCallerUserProfile, useGetRewardCampaigns } from '../hooks/useQueries';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { BarChart3, Users, TrendingUp, Coins, Activity, Target, Brain, Megaphone, Wallet, Gift } from 'lucide-react';
...
export default function GlobalDashboardPage() {
  ...
  const { data: campaigns = [], isLoading: campaignsLoading } = useGetRewardCampaigns();
  ...
  const metrics = useMemo(() => {
    ...
    const userPendingCampaigns = userProfile?.pendingRewardCampaigns?.length ?? 0;

    // Total reward pool across all campaigns (in ICP e8s)
    const totalRewardPool = campaigns.reduce((sum, c) => sum + Number(c.rewardAmount), 0);
    ...
    <div className="container py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <Card className="bg-gradient-to-br from-chart-4/10 to-chart-5/10 border-chart-4/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Active Reward Campaigns</CardTitle>
          ...
          <CardContent>
            {campaignsLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">Live campaigns on platform</p>
              </>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-chart-5/10 to-primary/10 border-chart-5/20">
          ...
          <CardContent>
            {campaignsLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{metrics.totalRewardPoolICP} ICP</div>
        ...
        <Card className="bg-gradient-to-br from-accent/10 to-chart-2/10 border-accent/20">
          ...
          <CardContent>
            {campaignsLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{metrics.totalParticipants.toLocaleString()}</div>
          ...
          {campaignsLoading ? (
            ...
            <div className="space-y-3">
              {campaigns.slice(0, 5).map((campaign) => {
                ...
                const isUserPending = userProfile?.pendingRewardCampaigns?.includes(campaign.id) ?? false;
      ...
      <Card className="mb-8">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Reward Campaign Participants</span>
              {campaignsLoading ? (
                <Skeleton className="h-8 w-16" />
              ) : (
                <span className="text-2xl font-bold text-chart-2">{metrics.totalParticipants.toLocaleString()}</span>
              )}
            </div>
            <div className="flex items-center justify-between">
              ...
              {campaignsLoading ? (
                ...
                <span className="text-2xl font-bold text-chart-3">{metrics.userPendingCampaigns}</span>
              )}
            </div>
          </div>
        </CardContent>
