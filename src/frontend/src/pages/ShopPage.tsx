...
import { type CreateOrderInput, useCreateOrder } from "../hooks/useQueries";
...
export default function ShopPage() {
  ...
  const handleOrderClick = async (product: Product, quantity: number) => {
    ...
    try {
      ...
      const vendorName =
        vendorMap.get(product.vendorId.toString()) ?? "Unknown Vendor";
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          ...
          <Select value={vendorFilter} onValueChange={setVendorFilter}>
            ...
            <SelectContent>
              ...
              {vendors.map((v) => (
                <SelectItem
                  key={v.principalId.toString()}
                  value={v.principalId.toString()}
                >
                  {v.displayName}
                </SelectItem>
              ))}
