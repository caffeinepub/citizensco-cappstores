...
import type { RevenueShareParticipant } from "../types";
...
export default function RevenueShareConfigModal({
  open,
  onClose,
}: RevenueShareConfigModalProps) {
  ...
  const totalPercentage = participants.reduce((sum, p) => {
    const val = Number.parseFloat(p.percentage);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const p of participants) {
      const pct = Number.parseFloat(p.percentage);
      ...
      if (!participant.principal && !participant.stripeId) {
        toast.error(
          "Each participant must have either a Principal ID or Stripe ID",
        );
        return;
      }
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Define how revenue is distributed among participants. Total must
            equal 100%.
          </DialogDescription>
        </DialogHeader>
              ...
              {p.idType === "principal" ? (
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    value={p.principal}
                    onChange={(e) =>
                      updateParticipant(p.id, "principal", e.target.value)
                    }
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    value={p.stripeId}
                    onChange={(e) =>
                      updateParticipant(p.id, "stripeId", e.target.value)
                    }
                    className="font-mono text-xs"
                  />
                </div>
